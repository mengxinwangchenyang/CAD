...
def handle_entity(e, rows, space_name: str, depth=0, group_key: Optional[str] = None, module_key: Optional[str] = None, ctx: Optional[dict] = None):
    dxftype = e.dxftype()
        ...
        if depth < 10:
            try:
                for i, ve in enumerate(e.virtual_entities()):
                    ...
                    handle_entity(ve, rows, space_name, depth + 1, group_key=gkey, module_key=raw_mkey, ctx=ctx)
            except Exception:
                pass
        return
